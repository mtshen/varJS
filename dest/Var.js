  ...
  */
	Global.prototype.parseJson = function (data) {
		...
		if (typeof data === 'string') {
			data = data !== '<html><body><h1>内部服务器错误</h1></body></html>' ? JSON.parse(data) : false;
		} else if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' && data.code === 10) {
			data = false;
		}
		if (!data) {
  ...
  */
	Global.prototype.is = function (content) {
		try {
			...
			if ((typeof content === 'undefined' ? 'undefined' : _typeof(content)) !== "object") return typeof content === 'undefined' ? 'undefined' : _typeof(content);
			if (content.nodeType) return "domElement";
			if ((typeof content === 'undefined' ? 'undefined' : _typeof(content)) === "object" && Object.prototype.toString.call(content) === "[object Array]") return "array";
			if (content.length && typeof content.length === "number" && content.length > -1) return "likeArray";
			if ((typeof content === 'undefined' ? 'undefined' : _typeof(content)) === "object") return Object.prototype.toString.call(content).replace(/\[object (\w+)\]/, "$1").toLowerCase();
...
Var.prototype.getTemplate = function () {
	...
	function __tpl_ifElseFn(templateHtml) {
		...
		var tpl_html = tpl_regexp[0].replace('\'', '\"').replace(/\{\{ ?if ?([^{}]+)\}\}/, 'if($1){\'').replace(/\{\{ ?else ?([^{}]+)\}\}/g, '\'}else if($1){\'').replace(/\{\{ ?else ?\}\}/, '\'} else {\'').replace(/\{\{ ?if ?\}\}/, '\'}');
...
Var.prototype.addMethods('merge', function () {
	...
	return function () {
		...
		switch (typeof objArrsPop === 'undefined' ? 'undefined' : _typeof(objArrsPop)) {
			case 'number':
				addNum = objArrs.pop();
				break;
			case 'string':
...
Var.prototype.addMethods('isObject', function (str) {
	return (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === "object" && obj !== null || typeof obj === "function";
});
/**
 * @functionName isDOM
 * 判断传入的内容是否是dom元素
